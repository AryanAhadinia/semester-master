		...
		return (
			<React.Fragment>
				{this.state.times.map((time) => (
					<div
						className='course-card d-flex flex-column align-items-center justify-content-around'
						style={{
							gridRow:
								this.state.rows[time] +
								'/ span ' +
								this.state.durations[time],
							gridColumn: this.state.columns[time],
							backgroundColor: 'rgba(232, 73, 48, 0.3)',
							zIndex: this.state.indexes[time],
							marginLeft: this.state.indexes[time] / 2 + 'vw',
							marginRight: -(this.state.indexes[time] / 2) + 'vw',
							marginTop: this.state.indexes[time] / 2 + 'vw',
							marginBottom:
								-(this.state.indexes[time] / 2) + 'vw',
						}}
					>
						...
						<h2 className='class-attributes'>
							{this.state.course.courseId +
								'-' +
								this.state.course.groupId}
						</h2>
