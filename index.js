...
function authenticate(req, res, next) {
	...
	jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, function (err, user) {
		...
		req.user = user;
		res.cookie('token', getToken(user.email, user.role), {
			maxAge: tokenExpiry,
			httpOnly: true,
		});
		next();
...
function validateCourseMiddleware(req, res, next) {
	...
	if (
		...
		return res
			.status(400)
			.send('فیلد های مربوط به صورت کامل مقدار دهی نشده اند');
	}
...
function validateTimeObject(timeObject) {
	...
	if (
		!Number.isInteger(timeObject.weekday) ||
...
app.post(
	...
	body('password')
		.isLength({ min: 8 })
		.withMessage('رمز عبور باید دست کم شامل 8 کاراکتر باشد'),
	function (req, res) {
...
app.get('/api/schedule/courses', function (req, res) {
	MongoClient.connect(dbURL, function (err1, db) {
		...
		db.collection('Course')
			.find({})
			.toArray(function (err2, allCourses) {
				if (err2) {
					res.sendstatus(500);
				} else {
...
app.get('/api/schedule/departments', function (req, res) {
	MongoClient.connect(dbURL, function (err1, db) {
		...
		db.collection('Course').distinct(
			'department',
			{},
			function (err2, departments) {
				if (err2) {
					res.sendstatus(500);
				} else {
...
app.put(
	...
	function (req, res) {
		...
		MongoClient.connect(dbURL, function (err1, db) {
			...
			db.collection('Selection').update(
				selectObject,
				selectObject,
				{ upsert: true },
				function (err2, input) {
...
*/
app.get('/api/schedule/selections', authenticate, function (req, res) {
	MongoClient.connect(dbURL, function (err1, db) {
		if (err1) {
			res.sendStatus(500);
			try {
		...
		db.collection('Selection')
			.find({ email: req.user.email }, { _id: 0 })
			.toArray(function (err2, mySelections) {
				if (err2) {
					res.sendStatus(500);
				} else {
		...
		MongoClient.connect(dbURL, function (err1, db) {
			...
			db.collection('User').findOne(
				...
				function (err2, user) {
					...
					if (
						user.password == sha256(passwordSent) &&
						user.role == 'admin'
					) {
						res.cookie('token', getToken(emailSent, user.role), {
							maxAge: tokenExpiry,
...
app.put(
	...
	body('courseId')
		.isInt({ min: 20000, max: 100000 })
		.toInt()
		.withMessage('کد درس معتبر نیست'),
	body('groupId')
		.isInt({ min: 1, max: 100 })
		.toInt()
		.withMessage('شماره گروه درس معتبر نیست'),
	body('unit')
		.isInt({ min: 0, max: 4 })
		.toInt()
		.withMessage('تعداد واحد معتبر نیست'),
	function (req, res) {
		const errors = validationResult(req);
		if (!errors.isEmpty()) {
			return res.status(400).send(errors.array()[0]['msg']);
		...
		MongoClient.connect(dbURL, function (err1, db) {
			...
			db.collection('Course').deleteMany(
				...
				function (err2, deleted) {
					...
					db.collection('Selection').deleteMany(
						queryObject,
						function (err3, deletedSelections) {
							if (err3) {
								res.sendStatus(500);
							} else {
								res.sendStatus(200);
...
DEL	:: /api/admin/dropsemester		admintoken
    ...
    403 : no message			    FORBIDEN
*/
app.delete('/api/admin/dropsemester', authenticateAdmin, function (req, res) {
	MongoClient.connect(dbURL, function (err1, db) {
		if (err1) {
			res.sendStatus(500);
			try {
...
GET	:: /api/admin/crawl				urlencoded: eduToken; admintoken
    ...
    // TODO
*/
app.put('/api/admin/crawl', authenticateAdmin, function (req, res) {});
...
app.get('/forgetpass/serve/:token', function (req, res) {
	if (!req.params['token']) {
		res.sendStatus(400);
		return;
	}
	...
	jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, function (err, user) {
		...
		res.sendFile(
			path.join(__dirname, '/public/change_password/index.html')
		);
	});
});

app.get('/api/account/validate', authenticate, function (req, res) {
